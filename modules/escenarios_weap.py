...
def renderizar_motor_escenarios_weap(territorio="Territorio Global"):
    ...
    # 3. MOTOR MATEMÁTICO DE SIMULACIÓN
    # Aplicamos los factores de estrés a los valores base
    oferta_modificada = oferta_base_m3s * (1 + (var_clima / 100))
    demanda_modificada = demanda_base_m3s * (1 + (var_pob / 100)) * (1 - (var_eficiencia / 100))

    # Simulamos una curva anual básica (estiajes en inicio y mitad de año)
    ...
    demanda_mensual = np.full(12, demanda_modificada) # Asumimos demanda constante todo el año

    # 4. GRÁFICA DE BALANCE (Estilo WEAP: Unmet Demand)
    fig = go.Figure()

    # Área de Oferta (Azul)
    fig.add_trace(go.Scatter(
        x=meses, y=oferta_mensual,
        mode='lines',
    ...
    # Línea de Demanda (Naranja)
    fig.add_trace(go.Scatter(
        x=meses, y=demanda_mensual,
        mode='lines',
        name='Demanda Total',
        line=dict(color='#e67e22', width=3, dash='dash')
    ))

    # 🚀 MAGIA WEAP: Pintar de rojo el área donde Demanda > Oferta (Déficit)
    # Rellenamos el espacio entre las curvas solo cuando hay déficit
    ...
    fig.add_trace(go.Scatter(
        x=meses, y=oferta_minima_grafico,
        mode='lines',
        name='Déficit Hídrico (Unmet Demand)',
        line=dict(width=0),
        fill='tonexty', 
        fillcolor='rgba(231, 76, 60, 0.5)' # Rojo semitransparente
    ))
    ...
    # 5. DIAGNÓSTICO ESCRITO AL VUELO
