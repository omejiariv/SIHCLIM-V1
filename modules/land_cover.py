...
# --- 2. FUNCIONES AUXILIARES CLOUD ---

def _cloud_path(path):
    """Añade el prefijo virtual de GDAL si es una URL pública."""
    if path and path.startswith("http"):
        return path.replace("https://", "/vsicurl/https://").replace("http://", "/vsicurl/http://")
    return path

def get_pixel_area_in_km2(transform, crs, height, width):
    """Calcula el área de un píxel en km²."""
    px_w, px_h = abs(transform[0]), abs(transform[4])
    if crs.is_geographic:
        return (px_w * 110.5) * (px_h * 111.32)
    return (px_w * px_h) / 1_000_000

# --- 3. PROCESAMIENTO ROBUSTO ---

def process_land_cover_raster(raster_path, gdf_mask=None, scale_factor=1):
    if not raster_path: return None, None, None, None
    env_kwargs = {'GDAL_DISABLE_READDIR_ON_OPEN': 'EMPTY_DIR', 'CPL_VSIL_CURL_ALLOWED_EXTENSIONS': 'tif'}
    
    try:
        with rasterio.Env(**env_kwargs):
            with rasterio.open(_cloud_path(raster_path)) as src:
...
def get_land_cover_at_point(lat, lon, raster_path):
    if not raster_path: return "Raster no encontrado"
    try:
...
# --- INTEGRACIÓN CON HIDROLOGÍA (TURC) ---
def calculate_cover_stats(gdf_geom, raster_path):
    if gdf_geom is None or gdf_geom.empty or not raster_path: return {}
    try:
        with rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN='EMPTY_DIR', CPL_VSIL_CURL_ALLOWED_EXTENSIONS='tif'):
            with rasterio.open(_cloud_path(raster_path)) as src:
...
def obtener_imagen_folium_coberturas(gdf_zona, raster_path):
    if gdf_zona is None or not raster_path: return None, None
    try:
        with rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN='EMPTY_DIR', CPL_VSIL_CURL_ALLOWED_EXTENSIONS='tif'):
            with rasterio.open(_cloud_path(raster_path)) as src:
...
def obtener_vector_coberturas_ligero(gdf_zona, raster_path, max_poly=1000):
    if gdf_zona is None or not raster_path: return None
    try:
        with rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN='EMPTY_DIR', CPL_VSIL_CURL_ALLOWED_EXTENSIONS='tif'):
            with rasterio.open(_cloud_path(raster_path)) as src:
                if gdf_zona.crs != src.crs: gdf_zona = gdf_zona.to_crs(src.crs)
